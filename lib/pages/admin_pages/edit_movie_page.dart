...
class _EditMoviePageState extends State<EditMoviePage> {
  ...
  void initState() {
    ...
    _sinopsisController = TextEditingController(text: widget.movie.sinopsis);
    ...
    if (widget.movie.releaseDate != null &&
      ...
      try {
        final parsedDate = DateTime.parse(widget.movie.releaseDate!);
        formattedDate =
            "${parsedDate.year.toString().padLeft(4, '0')}-${parsedDate.month.toString().padLeft(2, '0')}-${parsedDate.day.toString().padLeft(2, '0')}";
      } catch (e) {
        formattedDate = widget.movie.releaseDate!;
      }
    }
    _releaseDateController = TextEditingController(text: formattedDate);

    _existingPosterUrl = widget.movie.posterUrl;
  }
  ...
  // Updated function to handle movie update
  Future<void> _updateMovie() async {
    ...
    try {
      ...
      await MovieService.updateMovie(widget.movie.idMovie!, updatedMovieData);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to update movie: ${e.toString()}')),
        );
      }
    } finally {
              ...
              children: [
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                      hintText: _posterFileName ?? 'Current poster is set',
